...
@section('content')
    ...
    <div class="page-body pt-6">
        <div class="container-xl">
        <a href="{{ LaravelLocalization::localizeUrl(route('dashboard.admin.ads.adsAddOrUpdate')) }}" class="btn btn-success">
			...
			<div class="card">
				<div id="table-default" class="card-table table-responsive">
					<table class="table">
						...
						@foreach($ads as $entry)
                            <tr>
                                <td>{{ $entry->title }}</td>
                                <td>{{ $entry->ad_type_id }}</td>
                                <td>{{ $entry->redirected_url }}</td>
                                <td>{{ $entry->show_count }}</td>
                                <td>{{ $entry->click_count }}</td>
                                <td> @if($entry->role != 'default')
                                            <a href="{{ LaravelLocalization::localizeUrl( route('dashboard.admin.ads.adsAddOrUpdate', $entry->id) ) }}" class="btn w-[36px] h-[36px] p-0 border hover:bg-[var(--tblr-primary)] hover:text-white" title="{{__('Edit')}}">
                                            ...
                                            <a href="{{ LaravelLocalization::localizeUrl( route('dashboard.admin.ads.delete', $entry->id) ) }}" onclick="confirm('{{__('Are you sure? This is permanent.')}}')" class="btn p-0 border w-[36px] h-[36px] hover:bg-red-600 hover:text-white" title="{{__('Delete')}}">
